...
type PositionForm = { strokeSystem: StrokeSystem; strokes: string; element: ElementValue | "" };
type FormState = {
  surname: string;
...
const emptyPosition = (): PositionForm => ({ strokeSystem: "kangxi", strokes: "", element: "" });
...
function Results({ data }: { data: ResponseShape }) {
  return <section className="results-section" id="results" aria-labelledby="results-title">
    ...
    {data.results.length === 0 ? <div className="empty-state"><div className="empty-mark"><Icon name="search" size={28} /></div><h3>目前沒有完全符合的名字</h3><p>試著放寬指定筆畫或五行條件，再重新產生一次。</p><a href="#solver">返回調整條件</a></div> :
    ...
    <p className="result-disclaimer"><Icon name="info" size={17} /> 結果依目前測試資料與指定規則計算，適合作為探索靈感，不作戶政登記或命理判定依據。</p>
  </section>;
}
...
export default function HomePage() {
  ...
  function updateLength(length: 1 | 2 | 3) {
    setForm((current) => ({ ...current, givenNameLength: length, positions: Array.from({ length }, (_, index) => current.positions[index] ?? emptyPosition()) }));
  }
  ...
  async function submit() {
    setError(undefined);
    if (mode === "guided" && !form.surname.trim()) {
      setError("請先輸入姓氏，再產生名字推薦。"); document.getElementById("surname")?.focus(); return;
    }
    setLoading(true);
    try {
      const constraints = mode === "json" ? JSON.parse(jsonInput) : buildConstraints(form);
      const response = await fetch("/api/v1/solve", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ constraints, limit: 12 }) });
      const data = await response.json();
      if (!response.ok) {
        const issue = data.issues?.[0];
        throw new Error(issue ? `條件格式有誤：${issue.message}` : data.error === "rate_limited" ? "操作太頻繁，請稍後再試。" : "目前無法完成計算，請檢查條件後重試。");
      }
      setResult(data); window.setTimeout(() => resultsRef.current?.focus(), 50);
    } catch (reason) {
  ...
  return <>
      ...
      <section className="solver-shell" id="solver" aria-labelledby="solver-title">
        ...
        <div className="workspace-layout"><div className="form-column">
          {mode === "guided" ? <>
            ...
            <section className="form-section" aria-label="每字條件"><SectionHeading number="02" title="每字條件" description="不確定時維持「不限」即可。" /><div className="positions-stack">{form.positions.map((position, index) => <PositionEditor key={index} index={index} value={position} onChange={(value) => updatePosition(index, value)} />)}</div><details className="advanced-options"><summary>更多硬性條件 <span>總筆畫、避用字</span></summary><div className="field-grid"><div className="field"><label htmlFor="total-strokes">名字總筆畫 <span>選填</span></label><input id="total-strokes" inputMode="numeric" value={form.totalStrokes} onChange={(event) => setForm({ ...form, totalStrokes: event.target.value })} placeholder="例如 14、18" /><small>僅計算名字，不含姓氏</small></div><div className="field"><label htmlFor="excluded-chars">完全避用的字 <span>選填</span></label><input id="excluded-chars" value={form.hardExcludedChars} onChange={(event) => setForm({ ...form, hardExcludedChars: event.target.value })} placeholder="例如：豪、強" /><small>可直接連續輸入多個中文字</small></div></div></details></section>
          ...
          <button type="button" className="submit-button" onClick={submit} disabled={loading}>{loading ? <><span className="spinner" /> 正在整理候選名字…</> : <>產生名字推薦 <Icon name="arrow" /></>}</button><p className="submit-help">通常在一秒內完成 · 不會儲存個人姓名資料</p>
    ...
    <footer><div className="footer-brand"><span className="brand-mark" aria-hidden="true">名</span><p><strong>NameSolver</strong><br />確定性、可追溯的名字條件探索工具</p></div><p>資料版本 fixtures-1.0.0 · 僅供開發測試與名字靈感探索</p></footer>
  </>;
}
