...
function score(result: SolveResult, constraints: ConstraintV1): ScoreBreakdown {
  const characters = result.characters;
  const rarity = constraints.preferences.rarity
    ? characters.reduce((sum, character) => sum + (character.rarityBand === constraints.preferences.rarity ? rarityPoints[character.rarityBand] : 0), 0)
    : 0;
  ...
  const easeOfInput = constraints.preferences.avoidDifficultCharacters
    ? Math.round(characters.reduce((sum, character) => sum + (1 - character.inputDifficulty) * 8, 0))
    : 0;
...
export function featureFacts(character: Character): string[] {
  return [
    `稀有度：${character.rarityBand}`,
    `輸入難度：${Math.round(character.inputDifficulty * 100)}%`,
